...
```
p-value of the test is less than 0.05. We reject the H0 hypothesis in favor of hypothesis H1 and we conclude that education of higher educated parent and mathematics grade are dependent attributes.
...
chisq.test(tbl,correct=F)
```
p-value of the independence test is less than 0.05. Therefore, we reject the H0 hypothesis in favor of H1 hypothesis. We conclude that mother's education and mathematics grade are dependent.
...
```
p-value of the test is less than 0.05. We reject the H0 hypothesis in favor of hypothesis H1 and we conclude that education of higher educated parent and Portuguese grade are dependent attributes.
...
p-value of the test is higher than 0.05. We do not reject the H0 hypothesis in favor of hypothesis H1.
...
Mean grades for each subject will be used to decide a direction (left or right) of one-sided t-test. School with higher mean grade will be in favor of alternative hypothesis H1.
...
## Mathemathics - relative frequencies and mean
```{r, colapse=TRUE, echo=FALSE}
...
## Portuguese - relative frequencies and mean
...
```
Tails are emphasised on the left side of the distribution, which is why the `p` value will almost always be less than 0.05 for the Kolmogovor-Smirnov and Lilliefors' test. 
...
cat_reject_h0 <- function(prefix_message, is_h0_rejected) {
  cat(prefix_message, "\n")
  if (is_h0_rejected) cat("\tWe reject the hypothesis H0 in favor of hypothesis H1\n") else cat("\tWe do not reject the hypothesis H0\n")
}
```
...
Intuitively, it can be assumed that the hypothesis H0 for Portuguese will be rejected because the variances are significantly different from each other. Of course, F-test of equality of variances has to be conducted to assure the statistical significance between the two variances.
...
Mathemathics - hypothesis H0 is not rejected. It can't be stated that school GP has better math grades than scchool MS

Portuguese - hypothesis H0 in favor of hypothesis H1 from which it's concluded that school `GP` has better grades in Portuguese than school `MS`.
