...
Saznajemo osnovne podatke za svaki stuapc 
```{r, colapse=TRUE, results='hide'}
...
Provjeravamo postoje li nevažeći podaci koji prelaze maksimalne vrijednosti specificirane u uputama o podacima. Sve vrijednosti su dobrom intervalu.
```{r, colapse=TRUE, results='hide'}
...
# one hot encodeaj kateogricke varijable
# sad stupci vise nisu vrijednosti char nego su 1 ili 0 
# npr. MALE 0 (zensko)
# MALE 1 (musko)

students_dummy = dummy_cols(students_char, charcols, remove_selected_columns = TRUE)
# ako ti treba, cor tablica radi!
# cor(students_dummy)
...
# Petar Dragojević
Ispitati zavisnost izmedu edukacije roditelja i uspješnosti.
```{r}
...
boxplot(students_org$G3_mat, students_org$G3_por, 
        names = c('konačna ocjena iz matematike','konačna ocjena iz portugalskog'),
        main='Boxplot konačnih ocjena iz matematike i portugala')
